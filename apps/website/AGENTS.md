...
- **Customer management routes**: customers use dedicated add/edit pages (`/customers/add`, `/customers/[id]/edit`) that mirror product add/edit pages. Add/edit navigation uses anchors styled with `buttonVariants`, not click handlers that call `goto` or open dialogs.
