...
- **Isolated development resources**: ignored root `.env` uses a dedicated development Turso database, development-only Better Auth secret, and localhost auth URL. SvelteKit's Cloudflare platform proxy selects Wrangler `env.dev`, whose remote `STORAGE` binding is the dedicated `stores4u-dev` R2 bucket. Never point normal development at production Turso, R2, or auth credentials.
...
- `scripts` — production and fresh-account Turso/Cloudflare deployment automation. See `scripts/AGENTS.md`.
