...
- **POS flow**: `/cart` (Point of Sale — build the cart via barcode scan or product search) → `/checkout` (review, attach a customer, place the order) → `/orders/[id]`. Cart state is shared client-side via `src/lib/cart.svelte.ts`, an in-memory Svelte 5 rune-store class (resets on full reload; not persisted). The customer is optional: walk-in (default, anonymous) or an existing/new customer attached as `customerId` (the `checkout` handler snapshots `customer_name`/`customer_phone` onto the order). `/checkout` has a client-side empty-cart guard that bounces back to `/cart`. The sidebar/CTA entry point is `Point of Sale` → `/cart`.
