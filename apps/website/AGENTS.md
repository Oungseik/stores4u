...
- **First-run gate**: `src/hooks.server.ts` exports `setupGate` (runs before `authHandle`). With no users, it redirects everything to `/setup` except `/setup`, `/health`, static assets, first-owner auth endpoints (`sign-up/email`, `sign-in/email`, `sign-in/social`, OAuth callbacks, verify/session reads), and `/rpc/setup/*` (the setup create mutation). With users but no shop, `/setup` stays reachable (along with `/signin`, verify/forgot-password, auth, and `/rpc/setup/*`) so an OAuth-created first owner can finish store setup. Once the shop exists, `/setup` and public email signup are closed.
