...
- **First-run gate**: `src/hooks.server.ts` exports `setupGate` (runs before `authHandle`). With no users, it redirects everything to `/setup` except `/setup`, `/health`, static assets, first-owner auth endpoints (`sign-up/email`, `sign-in/email`, `sign-in/social`, OAuth callbacks, verify/session reads), and `/rpc/setup/*` (the setup status query + create mutation). With users but no shop, `/setup` stays reachable (along with `/signin`, verify/forgot-password, auth, and `/rpc/setup/*`) so an OAuth-created first owner can finish store setup. Once the shop exists, `/setup` and public email signup are closed.
...
- **Setup is oRPC-driven**: `/setup` has no SvelteKit load/action — `orpc.setup.status` (query) returns `{ needsAccount }`, and `orpc.setup.create` (mutation) creates the first owner via `auth.api.signUpEmail` when needed and inserts the one shop row, mirroring the old setup action. The page uses tanstack/svelte-form + tanstack-query; the client `goto`s `/signin?setup=1` (owner just created) or `/` (owner already signed in). Reachability is owned by `setupGate` (see First-run gate); the handler is the auth/role trust boundary.
