...
- Oxlint is the only linter repo-wide: root `.oxlintrc.json` holds the rule set, every workspace runs `oxlint --fix .` through the Turbo `lint` task, and Biome / Prettier format only (Biome's linter is disabled). Oxlint covers JS/TS plus the `<script>` blocks of `.svelte` files; CSS, JSON, and Svelte template markup are not linted. Rules that oxlint enables by default but this repo keeps off until their existing violations are cleaned up: `no-shadow`, `no-underscore-dangle`, `no-await-in-loop`, `unicorn/consistent-function-scoping`, `unicorn/prefer-add-event-listener`, `unicorn/require-module-specifiers`.
- `no-unused-vars` is an error, so unused imports must be deleted by hand — oxlint has no automatic removal fix. Oxlint's `// oxlint-disable-next-line` directives also accept the legacy `eslint-disable-next-line` spelling, which vendored UI components still use.
...
- **Single-store deployment**: this app supports ONE store per deployment (not SaaS / multi-tenant). The store is set up once through the `/setup` wizard. There is no shop switcher, no per-store routing, no slug, and no `shop.userId` ownership link.
...
- **Link-first auth**: email delivery is not configured yet. Verification, recovery, magic-link, and invite flows continue to render copyable links and never depend on email delivery. Two-factor authentication is removed at this time. See `apps/website/AGENTS.md` Auth model.
...
- **Lightsail VM production (single server)**: production builds with `svelte-adapter-bun` and runs as a plain Bun service (`nix run .#website`, which executes `bun apps/website/build/index.js`) on an AWS Lightsail VM behind a TLS reverse proxy. Ignored root `.env.prod` persistently owns production credentials; on the VM it is loaded into the service environment together with `STORAGE_DIR` for uploaded files. Root `bun run deploy` loads that file, builds first, then applies committed Drizzle migrations to Turso — it never rotates credentials; code delivery to the VM and service restart are operator steps until a VM bootstrap script lands in `scripts/`. Do not run migrations on app startup or during normal development.
- **Reproducible isolated development**: `bun run setup` is the one-time interactive machine bootstrap. It authenticates Turso when needed, creates or reuses the dedicated `stores4u-dev` Turso database, writes development-only database/auth values to ignored root `.env`, prepares local upload storage, and applies committed migrations. `bun run dev` only starts localhost Vite; it must not provision or migrate. Never point normal development at production Turso or auth credentials.
- **Local-disk file storage**: uploads live in `STORAGE_DIR` on the server filesystem (dev default `apps/website/.storage`) and are served through `/storage/[...key]`. Disk is the only backend; there is no object-storage integration.
- **Store timezone**: `shop.timezone` (IANA name) defaults to the server runtime timezone and is user-selectable at `/setup` and editable by the owner in Settings → Business. DB stays UTC; only date boundaries (today/week/month) and chart day-buckets are computed in the store tz; display labels render the server-bucketed date key in UTC so they match regardless of the viewer's browser tz. Not per-user — one tz for the whole store.
