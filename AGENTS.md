...
- **Single SQLite database**: all data (better-auth + store domain) lives in one local SQLite file at `DATABASE_PATH` (default `databases/store.db`). Relative paths resolve from the repository root for Drizzle commands, the website, and maintenance scripts. No Turso, no per-store DB files.
