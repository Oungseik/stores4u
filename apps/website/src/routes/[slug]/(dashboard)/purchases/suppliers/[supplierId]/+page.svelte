<script lang="ts">
  ...
  function handleRowClick(invoice: PurchaseInvoiceItem) {
    if (invoice.invoiceFileId) {
      goto(`/${params.slug}/purchases/invoices/${invoice.invoiceFileId}`);
    }
  }
...
<div class="flex flex-col gap-6 p-4 md:gap-8 md:p-6">
  <AdminDashboardHeader
    breadcrumbs={[
      { label: "Dashboard", href: `/${shop.slug}` },
      { label: "Purchases", href: `/${shop.slug}/purchases` },
      { label: "Suppliers", href: `/${shop.slug}/purchases/suppliers` },
